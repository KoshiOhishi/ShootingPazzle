...
PSOutput main(VSOutput input)
{
	...
	//影テクスチャのUV値が0～1の範囲外だったら影を描画しない
	shadowWeight = (shadowUV.x < 0 || shadowUV.x > 1 || shadowUV.y < 0 || shadowUV.y > 1) ? 1 : shadowWeight;
	
	//シェーディングによる色で描画
	output.target0 = float4(shadecolor.rgb * texcolor.rgb * data.color.rgb * shadowWeight, shadecolor.a * texcolor.a * data.color.a);
