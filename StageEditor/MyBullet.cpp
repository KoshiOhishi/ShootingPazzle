...
void MyBullet::CheckCollision()
{
	...
	for (int i = 0; i < stage->blocks.size(); i++) {
		//ブロックが持つ各カプセル判定において
		for (int j = 0; j < stage->blocks[i]->GetCapsule().size(); j++) {
			float distance = 0;
			Vector3 normal = {};
			//次フレーム移動長さ
			...
			bool isHitRay2Capsule = Collision::CheckRay2Capsule(ray, stage->blocks[i]->GetCapsule()[j], &distance, nullptr, nullptr, &normal);
			//次のフレームで衝突するか
